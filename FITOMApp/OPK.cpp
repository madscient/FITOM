...
COPK::COPK(CPort* pt, UINT8 mode, UINT8 fsamp) : CSoundDevice(DEVICE_OPK, 8, fsamp, pt)
{
	SetReg(0x01, 0xff, 1);
	SetReg(0x84, 0x00, 1);
	ops = 2;
...
void COPK::UpdateFreq(UINT8 ch, const FNUM* fnum)
{
	...
	SetReg(0x52 + chofs[ch], (fnum->block << 5) | UINT8(fnum->fnum >> 6), 1);
	SetReg(0x62 + chofs[ch], UINT8(fnum->fnum << 2) & 0xf0, 1);
}
...
COPK2::COPK2(CPort* pt, UINT8 mode, UINT8 fsamp) : COPK(pt, mode, fsamp)
{
