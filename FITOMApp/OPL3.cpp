...
COPL3::COPL3(CPort* pt, UINT8 fsamp) :
...
void COPL3::UpdateVolExp(UINT8 ch)
{
	...
	for (int i=0; i<4; i++) {
		if ((1 << i) & carmsk[voice->AL & 0xf]) {
			UINT8 tl;
			UINT8 tmp;
			tl = CalcEffectiveLevel(evol, voice->op[i].TL) >> 1;
			attr->baseTL[i] = tl;
...
void COPL3::UpdateFnumber(UINT8 ch, int update)
{
	if (ch < chs) {
		...
		PseudoDT1[ch] = attr->GetFnumber(Fnum, SINT16(off1));
		PseudoDT2[ch] = attr->GetFnumber(Fnum, SINT16(off2));
	}
...
void COPL3::UpdateKey(UINT8 ch, UINT8 keyon)
{
	...
	for (int i = 0; i < 4; i++) {
		tmp = (GetReg(rop + 0x20 + dch + opmap[i], 0)) & 0xbf;
		SetReg(0x20 + dch + opmap[i], tmp | (keyon ? 0 : 0x20), 1);
		if (!keyon && ((1 << i) & carmsk[voice->AL & 0xf])) {
			UINT8 rr = ((parent && parent->GetSustain() && (carmsk[voice->AL & 0xf] & (1 << i))) ?
				GET_RV(voice, i) : (GET_SR(voice, i) ? GET_SR(voice, i) : GET_RR(voice, i))) & 0xf;
			UINT8 tmp = (GET_SL(voice, i) << 4) | rr;
			SetReg(rop + 0x80 + opmap[i] + dch, tmp);
...
COPL3_2::COPL3_2(CPort* pt1, CPort* pt2, UINT8 mode, UINT8 fsamp, UINT8 devtype) : CSpanDevice(NULL, NULL, fsamp)
{
	COPL* opl1 = new COPL(pt1, 0, fsamp, DEVICE_OPL3_2);
	COPL* opl2 = new COPL(pt2, 0, fsamp, DEVICE_OPL3_2);
	switch (mode) {
	case 0:
		for (int i=0; i<6; i++) {
			opl1->EnableCh(i, 0);
