...
UINT16 CMidiInst::Status3()
{
	...
	if (ch[midich]->IsThru() && ret) {
		...
		CMidiOut* pmo = Parent->GetMidiOut(pch->GetOutPort());
		UINT8 thrumsg[4];
...
UINT16 CMidiInst::Control()
{
	...
	case 0x00:	//Bank Select MSB
		if ((MsgBuf[2] & 0xf0) == 0x70) {
			UINT8 sub = MsgBuf[2] & 0xf;
			if ((sub == 8 && ch[midich]->IsInst())  || (sub == 5 && ch[midich]->IsThru())) {
				Release(midich);
				AssignRhythm(midich);
				break;
			} else
			if ((sub == 9 && ch[midich]->IsRhythm()) || (sub == 4 && ch[midich]->IsThru())) {
				Release(midich);
				Assign(midich, NULL, 0);
				break;
			} else
			if (sub >= 0 && sub <= 3 && !ch[midich]->IsThru()) {
				Release(midich);
				AssignThru(midich, midich, (MsgBuf[2] & 0x3));
				break;
			}
		}
...
void CMidiInst::Assign(UINT8 nch, CSoundDevice* dev, UINT8 pol)
{
	if (nch < 16) {
...
{
	CSoundDevice* dev = Parent->GetInstDeviceFromID(bsl);
	if (dev) {
		//AllNoteOff();
...
void CInstCh::ProgChange(UINT8 prog)
{
	...
	if (Device)
	{
		Parent->GetVoice(&voice, Device->GetDevice(), UINT8(BankSelect), prog);
	...
	if (Device) {
		for (int i=0; i<timbres; i++) {
			...
			if (Portamento.IsEnable()) {
				Device->SetNote(Note[i].ch, Portamento.GetCurrentNote(), Portamento.GetCurrentFine()+Note[i].fine, 1);
			} else {
				Device->SetNote(Note[i].ch, Note[i].note, Note[i].fine, 1);
			}
		}
	}
}
...
UINT8 CInstCh::GetDeviceID() const
{
	UINT8 ret = 0xff;
	if (Device) {
		ret = Device->GetDevice();
	}
	return ret;	
}
...
{
	Volume = vol & 0x7f;
	for (int i=0; i<Parent->GetInstDevs(); i++) {
		CSoundDevice* dev = Parent->GetInstDeviceFromIndex(i);
		if (dev) {
...
void CRhythmCh::NoteOn(UINT8 note, UINT8 vel)
{
	...
	LastNote = note;
	FMVOICE dv;
	Parent->GetDrum(&dm, 0, UINT8(ProgramNo), note);
	if (dm.device) {
		if (Note[note].device) {
			NoteOff(note);
		}
