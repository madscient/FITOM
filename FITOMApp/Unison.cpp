...
CUnison::CUnison(CSoundDevice* chip1, CSoundDevice* chip2, UINT8 fsamp)
	: CMultiDevice(chip1, chip2, fsamp)
{
...
void CUnison::SetNote(UINT8 ch, UINT8 note, SINT16 fine, int update)
{
	for (int i=0; i<numchips; i++) {
		chips[i]->SetNote(ch, note, fine, update);
	}
}
...
void CUnison::GetDescriptor(char* str)
{
	if (numchips) {
		chips[0]->GetDescriptor(str);
	}
}
