...
COPL::COPL(CPort* pt, UINT8 mode, UINT8 fsamp, UINT8 devtype) :
CSoundDevice(devtype, 9, fsamp, pt), RhythmOnMap(0), RhythmOffMap(0)
{
	ops = 2;
	rhythmcap = 0;
	if (mode & 1) {
...
void COPL::UpdateVolExp(UINT8 ch)
{
	...
	if (voice->AL & 1) {
		tl = CalcEffectiveLevel(evol, voice->op[0].TL) >> 1;
		tmp = (UINT8)((voice->op[0].KSL << 6) | tl);
		attr->baseTL[0] = tl;
		SetReg(0x40 + map[ch], tmp);
	}
	tl = CalcEffectiveLevel(evol, voice->op[1].TL) >> 1;
...
void COPL::UpdateKey(UINT8 ch, UINT8 keyon)
{
	...
	UINT8 tmp;
	for (int i=0; i<2; i++) {
		tmp = (GetReg(0x20 + i * 3 + map[ch], 0)) & 0xbf;
		SetReg(0x20 + i * 3 + map[ch], tmp | (keyon ? 0 : 0x20), 1);
		UINT8 rr = ((parent && parent->GetSustain() && ((i==1) || (voice->AL&1))) ? GET_RV(voice, i) : ((keyon && GET_SR(voice, i)) ? GET_SR(voice, i) : GET_RR(voice, i))) & 0xf;
...
//--------
C3801::C3801(CPort* pt, UINT8 mode, UINT8 fsamp, UINT8 devtype) : COPL(pt, mode, fsamp, devtype)
{
}

//--------
COPL2::COPL2(CPort* pt, UINT8 mode, UINT8 fsamp, UINT8 devtype) : COPL(pt, mode, fsamp, devtype)
{
